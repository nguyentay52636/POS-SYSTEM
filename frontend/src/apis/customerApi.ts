...
export async function UpdatePointsToCustomer(customerId: number, request: AddPointsToCustomerRequest) {
  try {
    console.log(`[API] Adding points to customer ${customerId}:`, request)
    const { data } = await baseApi.put(`/Customer/${customerId}/points`, request)
